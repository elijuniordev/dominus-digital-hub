    ...
    <section className="py-20 bg-muted/30">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center mb-12">
          ...
          <Link to="/blog"><Button variant="outline" className="btn-outline-brand">Ver Todos os Posts<ArrowRight className="ml-2 h-4 w-4" /></Button></Link>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {isLoading ? (
            Array.from({ length: 3 }).map((_, index) => (
              <Card key={index}><CardHeader><Skeleton className="h-4 w-1/2 mb-4"/><Skeleton className="h-6 w-full"/></CardHeader><CardContent><Skeleton className="h-4 w-full"/><Skeleton className="h-4 w-full mt-2"/></CardContent></Card>
            ))
          ) : (
            latestPosts.map((post) => (
              <Link to={`/blog/${post.slug}`} key={post.id} className="block group">
                <Card className="card-elevated h-full hover:shadow-xl transition-all duration-300">
                  ...
                  <CardContent><CardDescription className="line-clamp-3">{createExcerpt(post.content)}</CardDescription></CardContent>
                </Card>
              </Link>
            ))
          )}
